...
prompt_list_t prompt_list_t_informal = {
    ...
    .question_fault =
        "Helaas klopt dat niet helemaal, waarschijnlijk ben je "
        "niet geschikt voor een opleiding aan de universiteit.",
...
prompt_list_t prompt_list_formal = {
    ...
    .math_float = "Geef het antwoord met twee decimalen na de komma.",
    .math_fault =
        "Helaas is dit fout, mogelijk is een beta studie iets voor "
        "u, de vragenlijst gaat door.",
    .question = "Verstrek het antwoord op de vraag. (a, b, c, d)",
    .question_fault =
        "Helaas is het antwoord fout, u bent waarschijnlijk niet "
        "geschikt voor een opleiding aan de universiteit.",
...
int main(int argc, char **argv) {
    ...
    date_prompt_item_t date_prompt_list[date_prompt_list_size] = {
        {.line = &prompt_list->year, .type = DATE_TYPE_YEAR},
        {.line = &prompt_list->month, .type = DATE_TYPE_MONTH},
        {.line = &prompt_list->day, .type = DATE_TYPE_DAY},
    };
    ...
    math_prompt_item_t math_prompt_list[math_prompt_list_size] = {
        {.line = &prompt_list->math_teller, .type = MATH_TYPE_ANSWER_TELLER},
        {.line = &prompt_list->math_noemer, .type = MATH_TYPE_ANSWER_NOEMER},
    };
    ...
    for (index = 0; index < date_prompt_list_size; index++) {
        ...
        if (rt_val == ERR_INVALID_INPUT) {
            ...
            printf(
                "De datum klopt niet of je bent to jong om deze "
                "vragenlijst in te "
                "vullen of .\n");
            return rt_val;
        }
