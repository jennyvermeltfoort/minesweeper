...
arg_t parse_args(int argc, const char* argv[]) {
    ...
    static unordered_map<string, int*> map_args = {
        ...
        {"-i", &args.robot_iterations}, {"--iterations", &args.robot_iterations},
    };
...
int main(int argc, const char* argv[]) {
    ...
    if (args.width < 1 || args.height < 1 || args.bomb_count < 1) {
        cout << "Usage: [-w,--width] <num> [-h,--height] <num> "
                "[-b,--bomb_count] <num> [-r,--robot] <output_path> [-i,--i] <num>"
             << endl;
        return 1;
    }
    ...
    if (!output_file.is_open()) {
    	...
    	input_handler.automated(args.width, args.height, args.bomb_count ,output_file, args.robot_iterations);
	output_file.close();
	return 0;
    }
    ...
    while (is_end == false) {
        char c = cin.get();
        input_handler.user_parse_input(board,c, &is_end);
    }
}
